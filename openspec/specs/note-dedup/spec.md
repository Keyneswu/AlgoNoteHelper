# note-dedup Specification

## Purpose
Soft near-duplicate detection and optional side-by-side resolve when importing or creating practice notes.
## Requirements
...
The system SHALL, after import extract (and when similarity matches exist for a candidate), surface a conflict affordance on that candidate row (for example a Conflict control placed before Remove). The affordance MUST be optional: if the user never opens resolve for that candidate, import commit MUST treat the candidate as create-as-new.

#### Scenario: Conflict affordance shown
...
The system SHALL provide a shared resolve UI (routed under `/notes/resolve` and/or `/import/resolve` using one component) where the user may select among up to three similar existing notes as the merge target. The UI MUST show incoming content as an editable draft (used for Keep as new) and the existing note as an editable result canvas. Empty existing fields MUST be prefilled from incoming. Non-empty existing fields MUST remain as the existing value until the user edits them or runs AI merge. The UI MUST offer Reverse to restore field snapshots from resolve entry (incoming and existing). The only completion actions SHALL be **Save** (merge into the selected existing note) and **Keep as new** (create a new note from the edited incoming draft). Completing either action MUST return the user to the originating Import or New Note task.
...
- **THEN** a new practice note is created from the (possibly edited) incoming draft (with unique-title handling), the existing note is unchanged, and the UI returns to the originating task
